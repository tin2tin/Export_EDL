...
bl_info = {
    ...
    "version": (0, 5),
    "blender": (2, 80, 0),
    "location": "Sequencer > Strip Sidebar",
...
def checkFPS():
    ...
    if fps in validFPS:
        ...
        bpy.context.window_manager.popup_menu(
            fr_error, title="Error", icon='ERROR')
        timecode = "timecode_%s" % str(fps).replace(".", "")
        """raise RuntimeError(
...
def fr_error(self, context):
    ...
    fps = round((render.fps / render.fps_base), 3)
    self.layout.label(
        text="Framerate \'" + str(fps) + "\' not supported by EDL. "
        "Change to 24, 25, 30, 60.")
...
#            self.hours = -self.hours
    ...
    def as_string(self):
        ...
        return "%.2d:%.2d:%.2d:%.2d" % (self.hours, self.minutes, self.seconds,
                                        self.frame)
    ...
    def saveEDL(self):
        ...
        for block in self:
            s += "%03d  %-8s %-4s  %-4s %03s %-11s %-11s %-11s %-11s" % (
                block.id, block.reel, block.channels, block.transition,
                block.transDur, block.srcIn, block.srcOut, block.recIn,
                block.recOut) + "\n"
            if block.file != "":
...
def write_edl(context, filepath, use_some_setting):
    ...
    strips_by_start_and_channel = sorted(
        strips_by_start, key=channel, reverse=True)
    jump = 0
    cnt = 0
    ...
    for strip in strips_by_start_and_channel:
        ...
        if strip.type in ['MOVIE'] and jump == 0 and video_okay:
            ...
            b.srcOut = TimeCode(
                strip.frame_offset_start + strip.frame_final_duration, edl_fps)
            ...
            b.transition = "C   " [0:4]
            b.transDur = "   "
            b.srcIn = TimeCode(realinput1.frame_offset_start +
                               realinput1.frame_final_duration, edl_fps)
            b.srcOut = TimeCode(realinput1.frame_offset_start +
                                realinput1.frame_final_duration, edl_fps)
            ...
            b.srcIn = TimeCode(
                realinput2.frame_offset_start - strip.frame_final_duration,
                edl_fps)
            # Source Inpoint of next clip + duration of next clip- duration of transition
            b.srcOut = TimeCode(
                realinput2.frame_offset_start +
                realinput2.frame_final_duration - strip.frame_final_duration,
                edl_fps)
